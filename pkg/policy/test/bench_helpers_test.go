...
// seedEndpoints allocates numIdentities identities and creates numEPs endpoints,
// assigning identities round-robin. Returns the created endpoints in creation
// order.
func seedEndpoints(tb testing.TB, ctx context.Context, f *testFixture, numEPs, numIdentities int) []*endpoint.Endpoint {
	...
	// Wait for each endpoint's initial regeneration (triggered by UpdateLabels)
	// to advance its policyRevision past 0. Otherwise a subsequent UpdatePolicy
	// may observe policyRevision==0 and take the "new endpoint" early-return
	// branch, leaving the endpoint stuck at an old revision.
...
// every identity seeded by seedEndpoints. The seed parameter makes the rule
// label unique across iterations so successive UpdatePolicy calls are distinct
// imports.
